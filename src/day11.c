...
static size_t count_paths(size_t l_id, size_t r_id, size_t level){
    if (l_id == r_id) { return 1; }
    size_t sum = 0;
    size_t l_idx = id_to_index[l_id];
    ...
    printf("id=%zu,idx:%zu =>\n", l_id, l_idx);
    for (size_t i = 0; i < graph[l_idx].n_edges; i++) {
        sum += count_paths(graph[l_idx].edges[i], r_id, level+1);
    }
    return sum;
}
...
char* day11_part2(const struct problem_inputs p_i){
  ...
  v_count = p_i.count;
    ...
    for (size_t i = 0; i < p_i.count; i++) {
        ...
        parse_buffer[3] = '\0';
        if (strcmp(parse_buffer, "out") == 0) {
            out_has_vertex = true;
        }

        v[i].name[0] = parse_buffer[0];
        v[i].name[1] = parse_buffer[1];
        v[i].name[2] = parse_buffer[2];
        v[i].name[3] = parse_buffer[3];
    ...
    printf("out_has_vertex: %s\n", bool2str(out_has_vertex));

    v[v_count].name[0] = 'o';
    v[v_count].name[1] = 'u';
    v[v_count].name[2] = 't';
    v[v_count].name[3] = '\0';
    v[v_count].id = pack_str("out");
    v[v_count].n_edges = 0;
    v[v_count].edges = NULL;
    v_count++;
    ...
    // size_t out_id = pack_str("out");
    // printf("out_id: %zu\n", out_id);
    // size_t out_idx = id_to_index[out_id];
    // printf("out_idx: %zu\n", out_idx);
