...
char* day4_part2(struct problem_inputs line_array) {
    int64_t total_jolts = 0;
    // const size_t DAY_04_PART_02_NUMBER_OF_BANKS = 12;

     for (size_t line_number = 0; line_number < line_array.len; line_number++)
        {
        char* current = line_array.inputs[line_number];

        size_t m_pos[DAY_04_PART_02_NUMBER_OF_BANKS];

        for (size_t i = 0; i < DAY_04_PART_02_NUMBER_OF_BANKS; i++) {
            m_pos[i] = i;
        }

        size_t len = strlen(current);
        size_t last_used_position = -1;
        size_t digits_left_to_pick = DAY_04_PART_02_NUMBER_OF_BANKS;

        for (size_t current_digit = 0; current_digit < DAY_04_PART_02_NUMBER_OF_BANKS; current_digit++) {
            digits_left_to_pick--;
...
static int64_t get_jolts(const size_t* pos_array, const char* str) {
    ...
    for (size_t i = 1; i < DAY_04_PART_02_NUMBER_OF_BANKS; i++) {
        c_idx = pos_array[i];
